<template>
  <div>
    ...
    <transition name="el-fade-in-linear">
      ...
      <div v-show="toolbar.visible" @mouseenter="hoverEnter = true" @mouseleave="hoverEnter = false" :class="[
        ...
        <!-- 手机抽屉按钮 -->
        <div v-if="$common.mobile() || mobile" class="toolbar-mobile-menu" @click="toolbarDrawer = !toolbarDrawer" :class="{ enter: toolbar.enter }">
        ...
        <div v-else>
          <ul class="scroll-menu">
            <!-- 文章 -->
            <li>
              <el-popover close-delay="100" popper-class="mk-popper" placement="bottom" trigger="hover">
  ...
  <!-- 切换背景 -->
  <li title="切换背景-换一种背景，换一种感觉。" id="changeThemeRef" style="color: var(--fontColor); font-size: 20px" @click="openChangeBg">
  ...
  <li>
    <el-dropdown placement="bottom">
      <el-avatar class="user-avatar" :size="36" style="margin-top: 4px" :src="
      ...
      <el-dropdown-menu slot="dropdown">
        <el-dropdown-item @click.native="$router.push({ path: '/user' })" v-if="!$common.isEmpty($store.state.currentUser)" class="user">
        ...
        <el-dropdown-item @click.native="$router.push({ path: '/user' })" v-if="$common.isEmpty($store.state.currentUser)" class="login">
    ...
    <el-popover placement="left" :close-delay="500" trigger="hover">
      ...
      <div class="my-setting">
        ...
        <el-tooltip placement="top" effect="light">
          ...
          <div>
            <i class="fa fa-snowflake-o" :class="{ active: mouseAnimation }" aria-hidden="true" @click="changeMouseAnimation()"></i>
          </div>
        </el-tooltip>
      </div>
    </el-popover>
  ...
  <el-dialog custom-class="changeBgBox" title="切换背景" :visible.sync="changeBgBox" :modal="false" width="60%" align="left">
    <div style="text-align: center">
      <button @click="defaultBtn" style="
            ...
            font-size: 18px;
          ">
        <i style="font-size: 18px; margin-right: 5px" class="fa fa-refresh"></i>点我恢复默认背景
      </button>
      <div class="customImg">
        <div class="customImg-item">设置自定义背景</div>
        <el-input v-model="httpInput" type="text" id="pic-link" size="70%" maxlength="1000" placeholder="请输入有效的图片链接，如 https://source.fomal.cc/img/home_bg.webp"></el-input>
        <button class="httpButton" @click="httpInputBtn">🌈切换背景🌈</button>
      </div>
      <div v-for="(items, index) in themeMap" :key="index">
        <h2 style="color:var(--fontColor);text-align: left">{{ items.title }}</h2>
        <div style="display: flex; align-items: center">
          ...
          <el-collapse @change="handleChangeBg(items.handleVal, index)">
            <el-collapse-item :title="items.collapseTitle" name="1">
              <div class="bgBox">
                <a @click="changeBg(item)" v-for="(item, i) in items.dataList" :key="i" href="javascript:;" :class="items.class" :style="
                      items.style === 'img'
...
export default {
  ...
  mounted() {
    ...
    if (localStorage.getItem('theme') == 'false') {
      ...
      root.style.setProperty('--fontColor', '#f7f7fa')
      ...
      root.style.setProperty('--fontColor', '#363636')
      root.style.setProperty('--borderColor', '#21232a')
  ...
  methods: {
    ...
    changeColor() {
      ...
      //false是黑夜，显示太阳图标
      if (!this.isDark) {
        roots.style.setProperty('--background', '#1e1e1e')
        ...
        roots.style.setProperty('--fontColor', '#363636')
        roots.style.setProperty('--borderColor', '#21232a')
        roots.style.setProperty('--borderHoverColor', 'rgba(110, 110, 110, 0.4)')
    ...
    changeBg(item) {
      ...
      if (!item && !JSON.parse(localStorage.getItem('vuex')).changeBg) {
        ...
        if (string[0] === '') {
          dom.setAttribute('style', `background-color: ${this.$store.state.changeBg}`)
        } else if (string.length === 1) {
          dom.setAttribute('style', `background-image: ${this.$store.state.changeBg};background-size: cover;background-attachment: local;background-position: center;width: 100%;height: 100%;`)
        } else {
          dom.setAttribute('style', `background-image: ${this.$store.state.changeBg}`)
        }
      } else if (item) {
        //只有点击切换时触发
        const dom = document.querySelector('.background-image-changeBg')
    ...
    defaultBtn() {
      const dom = document.querySelector('.background-image-changeBg')
      dom.setAttribute('style', 'background-image: url(https://www.qiniuyun.zjh2002.icu/images/changeBg3);background-size: cover;background-attachment: local;background-position: center;width: 100%;height: 100%;')
      this.$store.commit('changeBgBox', 'url(https://www.qiniuyun.zjh2002.icu/images/changeBg3)')
    },
    handleChangeBg(val, i) {
      ...
      if (val === 'mobile' && this.themeMap[i].dataList.length === 0) {
        ...
        this.$http
          .post(this.$constant.baseURL + '/resource/listResource/', pagination, true)
          .then(res => {
            if (!this.$common.isEmpty(res.result[0])) {
    ...
    getAllBg(i) {
      ...
      this.$http
        .post(this.$constant.baseURL + '/webInfo/allWebBackgroundImage/', pagination, true)
        .then(res => {
          if (!this.$common.isEmpty(res.result[0])) {
            res.result[0].records.forEach(item => {
              this.themeMap[i].dataList.push(item.path)
...
::v-deep .el-collapse {
  ...
  &-item__header {
    ...
    font-weight: 400;
    position: relative;
    border-bottom-color: var(--toolbarBackground);
    line-height: normal;
  }
  &-item__wrap {
    border-radius: 6px;
