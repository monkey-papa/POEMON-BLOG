<template>
  <div>
    ...
    <el-form :model="article" :rules="rules" ref="ruleForm" label-width="110px" class="demo-ruleForm">
      <el-form-item label="文章标题" prop="articleTitle">
        <el-input maxlength="30" v-model="article.articleTitle"></el-input>
      </el-form-item>
      <div style="color: var(--lightRed); padding-left: 34px; margin-bottom: 5px">
        默认文章作者：Monkey-PaPa
      </div>
      ...
      <el-form-item label="文章内容" prop="articleContent">
        <mavon-editor ref="md" @imgAdd="imgAdd" @imgDel="handleEditorImgDel" v-model="article.articleContent">
        </mavon-editor>
      </el-form-item>
      ...
      <el-form-item label="封面" prop="articleCover">
        <div style="display: flex">
          ...
          <el-image class="table-td-thumb" lazy style="margin-left: 10px" :preview-src-list="[article.articleCover]" :src="article.articleCover" fit="cover"></el-image>
        </div>
        <uploadPicture :isAdmin="true" :ResourceType="'articleCover'" style="margin-top: 10px" @addPicture="addArticleCover" :maxSize="3" :maxNumber="1"></uploadPicture>
      </el-form-item>
...
export default {
  ...
  methods: {
      ...
      this.$http
        ...
        .then(res => {
          ...
          if (!this.$common.isEmpty(res.url)) {
            ...
            //存取资源接口
            this.$common.saveResource(this, 'articlePicture', url, file.size, file.type, id, true)
          }
        })
        .catch(error => {
          this.$notify({
    ...
    handleEditorImgDel() {
      this.$http
        .post(this.$constant.baseURL + '/delArticleImage/', { url: this.url }, true)
        .then(res => {
          this.$notify({
            title: '可以啦🍨',
            message: '删除图片成功！',
    ...
    editArticle() {
      this.$http
        .get(this.$constant.baseURL + '/admin/article/getArticleById/', { id: parseInt(this.id) }, true)
        .then(res => {
          if (!this.$common.isEmpty(res.result[0])) {
