<template>
  <div>
    <div style="margin-bottom: 20px">
      <el-select @blur="closeOptions" ref="fuzzyCommentType" v-model="pagination.commentType" placeholder="评论来源类型" style="margin-right: 10px">
      ...
      <el-input class="my-input" type="number" style="width: 140px; margin-right: 10px" v-model="pagination.source" placeholder="评论来源标识"></el-input>
    ...
    <el-table :data="comments" border class="table" header-cell-class-name="table-header">
      ...
      <el-table-column label="头像" align="center">
        <template slot-scope="scope">
          <el-image lazy class="table-td-thumb" :src="scope.row.avatar || $store.state.webInfo.avatar" fit="cover"></el-image>
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    getComments() {
      this.$http
        .post(this.$constant.baseURL + '/admin/comment/boss/list/', this.pagination, true)
        .then(res => {
    ...
    handlePageChange(val) {
      if (this.pagination.commentType || String(this.pagination.source) != 'null') {
        this.$notify({
          type: 'error',
          title: '可恶🤬',
    ...
    doDelete(item) {
      this.$http
        .get(this.$constant.baseURL + '/admin/comment/boss/deleteComment/', { id: item.id }, true)
        .then(res => {
          this.pagination.current = 1
          this.getComments()
          this.$notify({
